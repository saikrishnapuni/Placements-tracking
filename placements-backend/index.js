...
app.post('/adminlogin', async(req, res) => {
    ...
    const database = db.getDb().db("Placements")
    try {
        const admins = await database.collection("admins").findOne({ username: req.body.uname })
        if (admins != []) {
            if (admins.password == req.body.pass) {
                database.collection('admins').updateOne({ username: req.body.uname }, { $set: { log: true } })
                res.send(true)
...
app.get("/logout", async (req, res) => {
    const database = db.getDb().db("Placements")
    await database.collection("admins").updateOne({ username: 'admin' }, { $set: { log: false } })
    res.send(true)
...
app.get("/recruiter/:id", async (req, res) => {
    ...
    if (recruitments.length > 0) {
        if (recruitments[0].selected_students) {
